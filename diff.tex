...
%DIF DEL nber-volume-vilhuber-old.tex   Mon Jan 19 12:05:20 2026
%DIF ADD nber-volume-vilhuber.tex       Mon Jan 19 12:05:26 2026
...
\begin{table}[]
    \centering
    \DIFdelbeginFL %DIFDELCMD < \input{latex/table-coverage}
%DIFDELCMD <     %%%
\DIFdelendFL \DIFaddbeginFL \input{table-coverage}
    \DIFaddendFL \caption{Distribution of Parameter-specific Confidence Interval Overlap, for selected projects}
    \label{tab:overlap}
\end{table}
...
In \DIFdelbegin \DIFdel{VILHUBER-HDSR}\DIFdelend \DIFaddbegin \DIFadd{\mbox{%DIFAUXCMD
\citet{vilhuber2025e}}\hskip0pt%DIFAUXCMD
}\DIFaddend , I demonstrate a simple scenario that satisfies most of the desiderata,  using containers. 
%
...
\DIFaddend While containers are not the ``normal'' way of developing statistical models in economics, they are increasingly being used in statistics, and at the cutting edge of the social sciences. \DIFaddbegin \DIFadd{\mbox{%DIFAUXCMD
\citet{boettiger_introduction_2015} }\hskip0pt%DIFAUXCMD
described the context of containers more  than 10 years ago, and the R community-maintained containers}\footnote{\url{https://hub.docker.com/r/rocker/}} \DIFadd{have more than 1 million downloads as of 2026. In the particular context described in this article, the system described by \mbox{%DIFAUXCMD
\citet{guimaraes_reproducibility_2023} }\hskip0pt%DIFAUXCMD
has been actively using containers for several years, with mostly economists as  prime clientele. Tutorials and graduate education have emerged in the past 5 years, if not longer, that teach the next generation of economists the necessary tools.}\footnote{\DIFadd{See the excellent syllabus by \mbox{%DIFAUXCMD
\citet{mcdermott2021}}\hskip0pt%DIFAUXCMD
, but also tutorials at the Carpentries \mbox{%DIFAUXCMD
\citep{eyers_reproducible_2020} }\hskip0pt%DIFAUXCMD
and the Turing Way \mbox{%DIFAUXCMD
\citep{the_turing_way_community_containers_2024}}\hskip0pt%DIFAUXCMD
.}} \DIFadd{In my own work as Data Editor for the }\ac{AEA}\DIFadd{, I have often successfully delegated to  undergraduates the task of figuring this out for themselves, and then teaching it to others, more senior \mbox{%DIFAUXCMD
\citep{graham2025}}\hskip0pt%DIFAUXCMD
.
}\DIFaddend 
...
I have benefited from discussions with many folks, including Gary Benedetto, John Abowd, Rob Sienkiewicz, and from feedback following presentations to the National Academies, Census Bureau, and at the NBER conference on ``Data Privacy Protection and the Conduct of Applied Research.'' The original development of the idea was partially funded by \DIFdelbegin %DIFDELCMD < \href{https://sloan.org/grant-detail/6845}{%%%
\DIFdelend \DIFaddbegin \href{https://sloan.org/grant-detail/g-2015-13903}{\DIFaddend Alfred P. Sloan Foundation Grant G-2015-13903}. The Synthetic Data Server project received funding from \href{https://www.nsf.gov/awardsearch/showAward?AWD_ID=1042181}{NSF grant SES-1042181}, \href{https://www.nsf.gov/awardsearch/showAward?AWD_ID=0941226}{NSF Grant BCS-0941226} and from the Alfred P. Sloan Foundation \DIFaddbegin \DIFadd{(}\href{https://sloan.org/grant-detail/g-2015-13903}{\DIFadd{G-2015-13903}}\DIFadd{) }\DIFaddend as well as the  Edmund Ezra Day chair at Cornell University.
...
\DIFdelbegin %DIFDELCMD < \bibliography{references,wsc2015,wsc2013,incomplete}
%DIFDELCMD < %%%
